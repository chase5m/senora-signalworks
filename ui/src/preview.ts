import type { ChaseSnapshot, ChaseStation } from "./types";
import { ChaseDetectProvider } from "./players";
let chaseSnapshot: ChaseSnapshot = {
  devices: {
    active: "buds",
    carry: "hand",
  ...
  stations: [
    {
      ...
      nowPlaying: {
        ...
        duration: 212,
        startedAt: Math.floor(Date.now() / 1000) - 84,
        paused: false,
        offsetSeconds: 84,
      },
      queue: [
  ...
  requests: [
    ...
    },
    {
      ...
      kind: "request",
      message:
        "Send some love to everyone finishing the late shift at Hayes tonight.",
      status: "pending",
    ...
    },
    {
      ...
      kind: "advertisement",
      message:
        "Bike meet at the canals. Roll through after midnight. Everyone welcome.",
      status: "pending",
  ...
  config: {
    ...
    powerModes: [
      ...
      { id: "high", label: "Citywide", range: 3000 },
    ],
...
function ChasePreviewSpeech() {
  ...
  chaseSnapshot.speech = {
    local:
      chaseMine?.micLive && chaseSnapshot.viewer.isHost
        ? {
            mode: "idle",
            talking: false,
            micOpen: true,
    ...
    receiver:
      chaseTuned && chaseTuned.id !== chaseMine?.id
        ? {
...
function ChasePreviewPlaced(payload: Record<string, unknown>) {
  const chaseNetId = Number(payload.placedNetId ?? payload.netId);
  const chasePlaced = chaseSnapshot.devices?.placedNearby?.find(
    (chaseItem) => chaseItem.netId === chaseNetId,
  );
  if (!chasePlaced) throw new Error("That radio is no longer nearby.");
  return chasePlaced;
}
function ChasePreviewRefreshPlace() {
  const chaseDevices = chaseSnapshot.devices;
  if (!chaseDevices) return;
  chaseDevices.canPlace =
    chaseDevices.owned.portable > 0 || chaseDevices.active === "portable";
}
function ChasePreviewStation(): ChaseStation {
  if (!chaseSnapshot.mine) throw new Error("Create a station first.");
  return chaseSnapshot.mine;
}
function ChasePreviewTrack(data: Record<string, unknown>) {
  ...
  if (
    !chaseProvider ||
    !chaseMusic?.enabled ||
    !chaseMusic.providers[chaseProvider]
  )
    throw new Error("Paste a supported YouTube or SoundCloud link.");
  const chaseDuration = Number(data.duration);
  if (
    !Number.isInteger(chaseDuration) ||
    chaseDuration < chaseMusic.minDurationSeconds ||
    chaseDuration > chaseMusic.maxDurationSeconds
  )
...
export async function ChasePreviewPost(
  endpoint: string,
  ...
  if (endpoint === "close" || endpoint === "volume" || endpoint === "talk")
    return {};
  if (endpoint === "bootstrap") return structuredClone(chaseSnapshot);
  const chaseAction = endpoint === "scan" ? "scan" : String(payload.action);
  const chaseData = (
    endpoint === "scan" ? payload : payload.data || {}
  ...
  if (chaseAction === "scan") {
    ...
    return {
      ...
      readings: 1,
      message:
        "Signal acquired. Move to a different location before taking another reading.",
    };
  }
  if (chaseAction === "equipDevice" && chaseSnapshot.devices) {
    const chaseDevice = String(chaseData.device) as
      "vehicle" | "portable" | "buds" | "none";
    if (!["vehicle", "portable", "buds", "none"].includes(chaseDevice))
      throw new Error("Unknown receiver.");
    if (
      chaseDevice !== "none" &&
      chaseDevice !== "vehicle" &&
      chaseSnapshot.devices.owned[chaseDevice] < 1
    )
    ...
    if (chaseDevice === "none") chaseSnapshot.tunedStationId = null;
    ...
    const chaseCarried =
      chaseDevices.active === "portable" ? chaseSnapshot.tunedStationId : null;
    const chaseStation = chaseSnapshot.stations.find(
      (chaseItem) => chaseItem.id === chaseCarried,
    );
    ...
    chaseSnapshot.devices.placedNearby = (
      chaseSnapshot.devices.placedNearby || []
    ).filter((chaseItem) => chaseItem.netId !== chasePlaced.netId);
    chaseSnapshot.devices.owned.portable += 1;
    ChasePreviewRefreshPlace();
  } else if (chaseAction === "tune") {
    const chaseStation = chaseSnapshot.stations.find(
      (chaseItem) =>
        chaseItem.id === chaseData.stationId ||
        chaseItem.frequency === chaseData.frequency,
    );
    if (!chaseStation?.live)
      throw new Error("No live transmission found on that frequency.");
    if (chaseData.placedNetId !== undefined) {
      const chasePlaced = ChasePreviewPlaced(chaseData);
    ...
    } else chaseSnapshot.tunedStationId = null;
    ...
    if (
      !Number.isInteger(chaseAmount) ||
      chaseAmount < 1 ||
      chaseAmount > chaseSnapshot.config.maxTip
    )
    ...
    const chaseUrl =
      chaseKind === "song" ? String(chaseData.url || "") : undefined;
    if (chaseUrl !== undefined && !ChaseDetectProvider(chaseUrl))
      throw new Error("Paste a supported YouTube or SoundCloud link.");
    const chaseLength =
      Array.from(chaseMessage).length +
      (chaseUrl === undefined ? 0 : Array.from(chaseUrl).length + 1);
    if (
      (chaseKind !== "song" && chaseLength < 3) ||
      chaseLength > Math.min(240, chaseSnapshot.config.requestMaxLength)
    )
      throw new Error(
        "The message and song link must fit within the request limit.",
      );
    if (chaseData.stationId === chaseSnapshot.mine?.id)
      chaseSnapshot.requests.unshift({
        ...
        kind: chaseKind as "request" | "song" | "message" | "advertisement",
    ...
    const chaseStation = chaseSnapshot.stations.find(
      (chaseItem) => chaseItem.id === chaseData.stationId,
    );
    const chaseHeardId =
      chaseData.placedNetId !== undefined
        ? ChasePreviewPlaced(chaseData).stationId
    ...
    if (
      ...
      chaseMine.pendingCall.callId !== chaseData.callId
    )
      throw new Error("That call is no longer waiting.");
    chaseMine.caller = chaseData.accept
      ? { name: chaseMine.pendingCall.callerName }
      : null;
    ...
    if (chaseAction === "updateStation") {
      ...
      const chaseRequest =
        chaseAction === "queueRequest"
          ? chaseSnapshot.requests.find(
              (chaseItem) =>
                chaseItem.id === chaseData.requestId &&
                chaseItem.stationId === chaseMine.id &&
                chaseItem.kind === "song" &&
                chaseItem.status === "pending",
            )
          : undefined;
      if (chaseAction === "queueRequest" && !chaseRequest?.url)
        throw new Error(
          "That song request was already handled or is unavailable.",
        );
      chaseQueue.push(
        ChasePreviewTrack(
          chaseRequest ? { ...chaseData, url: chaseRequest.url } : chaseData,
        ),
      );
      if (chaseRequest) chaseRequest.status = "accepted";
      chaseMine.queue = chaseQueue;
    } else if (chaseAction === "queueMove") {
      const chaseQueue = [...(chaseMine.queue || [])];
      const chaseIndex = chaseQueue.findIndex(
        (chaseTrack) => chaseTrack.id === chaseData.trackId,
      );
      const chasePosition = Number(chaseData.position);
      if (
        chaseIndex < 0 ||
        !Number.isInteger(chasePosition) ||
        chasePosition < 1 ||
        chasePosition > chaseQueue.length
      )
      ...
      chaseMine.queue = (chaseMine.queue || []).filter(
        (chaseTrack) => chaseTrack.id !== chaseData.trackId,
      );
    } else if (
      chaseAction === "playTrack" ||
      chaseAction === "skipTrack" ||
      chaseAction === "previousTrack"
    ) {
      if (!chaseMine.live) throw new Error("Start the broadcast first.");
      const chaseQueue = chaseMine.queue || [];
      let chaseIndex =
        chaseAction === "playTrack"
          ? chaseQueue.findIndex(
              (chaseTrack) => chaseTrack.id === chaseData.trackId,
            )
          : chaseQueue.findIndex(
              (chaseTrack) => chaseTrack.id === chaseMine.nowPlaying?.trackId,
            ) + (chaseAction === "previousTrack" ? -1 : 1);
      if (chaseAction === "previousTrack" && chaseIndex < 0)
        chaseIndex =
          chaseMine.mode === "autonomous" ? chaseQueue.length - 1 : 0;
      const chaseTrack =
        chaseQueue[
          chaseMine.mode === "autonomous" && chaseQueue.length
            ? chaseIndex % chaseQueue.length
            : chaseIndex
        ];
      ...
      if (chaseData.paused === true && !chasePlaying.paused)
        chasePlaying.offsetSeconds = Math.max(
          0,
          Date.now() / 1000 - chasePlaying.startedAt,
        );
      if (chaseData.paused === false && chasePlaying.paused)
        chasePlaying.startedAt =
          Date.now() / 1000 - (chasePlaying.offsetSeconds || 0);
      ...
      chaseAction === "playCartridge" ||
      chaseAction === "previewCartridge"
    ) {
      const chaseCartridge = chaseSnapshot.cartridges.find(
        (chaseItem) => chaseItem.id === chaseData.cartridgeId,
      );
      if (!chaseCartridge) throw new Error("Choose an installed cartridge.");
      if (chaseAction === "previewCartridge") {
        const chaseFiles: Record<string, string> = {
      ...
      const chaseRequest = chaseSnapshot.requests.find(
        (chaseItem) => chaseItem.id === chaseData.requestId,
      );
      if (chaseRequest)
        chaseRequest.status = chaseData.status as "accepted" | "dismissed";
      ...
      if (
        chaseSnapshot.crew.some((chaseItem) => chaseItem.source === chaseSource)
      )
      ...
      chaseSnapshot.crew = chaseSnapshot.crew.filter((chaseItem) =>
        chaseData.memberId
          ? chaseItem.memberId !== chaseData.memberId
          : chaseItem.source !== chaseData.source,
      );
      ...
      if (
        !Number.isInteger(chaseAmount) ||
        chaseAmount < 1 ||
        chaseAmount > (chaseMine.balance || 0)
      )
        throw new Error(
          "The withdrawal exceeds the available station balance.",
        );
      chaseMine.balance = (chaseMine.balance || 0) - chaseAmount;
    } else {
      throw new Error("This action is unavailable in the preview.");
    }
    chaseSnapshot.stations = chaseSnapshot.stations.map((chaseItem) =>
      chaseItem.id === chaseMine.id ? { ...chaseMine } : chaseItem,
    );
  }
  ChasePreviewSpeech();
